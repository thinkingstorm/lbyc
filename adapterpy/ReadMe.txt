...
Modifié dans la version 3.3: This function used to fail immediately when the import lock was held by another thread. In Python 3.3 though, the locking scheme switched to per-module locks for most purposes, so this function’s special behaviour isn’t needed anymore.
...
Modifié dans la version 3.3: Negative values for level are no longer accepted.
...
Modifié dans la version 3.3: Uses imp.source_from_cache() in calculating the source path if only the bytecode path is provided.
...
Modifié dans la version 3.4: The __file__ attribute is no longer set on the module.
